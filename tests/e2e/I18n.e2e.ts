...
test.describe('I18n', () => {
  test.describe('Language Switching', () => {
    test('should switch language from English to French using dropdown and verify text on the homepage', async ({ page }) => {
      ...
      await expect(page.getByText('The perfect Pride Logistics (P) Ltd.  to build')).toBeVisible();

      await page.getByRole('button', { name: 'lang-switcher' }).click();
      await page.getByText('Français').click();

      await expect(page.getByText('Le parfait Pride Logistics (P) Ltd.  pour construire')).toBeVisible();
    });
